...
//	@ExceptionHandler(value = PageException.class)
	...
	@ResponseBody
	public ApiResponse tokenErrorHandler(TokenException e) {
		StackTraceElement[] stackTrace = e.getStackTrace();
		StringBuilder sOut = new StringBuilder("【TokenException】:" + e.getMessage()+ "\r\n" + e.getClass().getName() + "\r\n");
		// 打印堆栈信息 前 x 条
		for (int i = 0; i< ConstantProperty.errorLogStackNumber ; i++) {
			sOut.append("\tat ").append(stackTrace[i]).append("\r\n");
		}
		log.error(sOut.toString());
		return ApiResponse.ofStatus(Status.UNAUTHORIZED);
	}
